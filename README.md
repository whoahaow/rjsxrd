...
Автоматически обновляемая коллекция публичных VPN-конфигов (`V2Ray` / `VLESS` / `Trojan` / `VMess` / `Reality` / `Shadowsocks`) для быстрого обхода блокировок. Обход белых списков на мобильном интернете.
...
githubmirror/        - корневая директория
 ├─ default/          - основные конфиги (1.txt, 2.txt, ..., 31.txt)
 └─ bypass/           - конфиги для обхода SNI/CIDR (bypass-all.txt, bypass-1.txt, bypass-2.txt, ...)
